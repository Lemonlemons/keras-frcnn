...
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='./model_frcnn_lions2.hdf5')
...
for epoch_num in tqdm(list(range(num_epochs)), desc='epoch'):
	try:
		for iter_num in tqdm(list(range(epoch_length)), desc='batch'):
			try:
		...
		iter_num = 0
		start_time = time.time()
		if epoch_num == 0 or curr_loss < best_loss:
			best_loss = curr_loss
			print('new best!')
		if C.verbose:
